...
const Gallery = () => {
  ...
  const fetchGalleryItems = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/product/latest-by-type`);
        setGalleryItems(response.data.latestProducts);
      } catch (error) {
              ...
              >
                {galleryItems.map((item, index) => (
                  <div
                    ...
                    style={{
                      filter: hoveredCard && hoveredCard !== item._id ? 'grayscale(100%)' : 'grayscale(0%)',
                      transform: hoveredCard === item._id ? 'scale(1.02)' : 'scale(1)',
                    }}
                  >
                    <img
                      src={item.image}
